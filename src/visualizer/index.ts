...
export default class Visualizer {
  ...
  private updateScrollingState(): void {
    ...
    // recorded click - and the post-record tail after it - could land just
    // past this and never render. Floor at audioBufferDurationMs - the
    // recording's *actual* length - never at scheduledDurationMs, which is
    // only a timer-scheduled estimate: if the real capture ever comes up
    // short of what was scheduled, flooring at the estimate would extend the
    // viewport into space with no audio at all, not just unanalyzed audio.
